...
func TestInMemoryRuleRegistry(t *testing.T) {
	...
	// Test RegisterRule
	rule1 := NewRequiresParentRule("Subnet", "VPC")
	err := registry.RegisterRule("Subnet", rule1)
	if err != nil {
		t.Errorf("Expected no error, got: %v", err)
	}

	rule2 := NewRequiresRegionRule("VPC", true)
	...
	if subnetRules[0].GetType() != RuleTypeRequiresParent {
		t.Errorf("Expected RuleTypeRequiresParent, got %v", subnetRules[0].GetType())
	}

	// Test GetRulesByType
	parentRuleType := RuleTypeRequiresParent
...
func TestDefaultRuleFactory(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			constraintValue: "VPC",
			expectedType:    RuleTypeRequiresParent,
			expectError:     false,
		},
		{
			name:            "allowed_parent",
			...
			expectedType:    RuleTypeAllowedParent,
			expectError:     false,
		},
		{
			...
			expectedType:    RuleTypeRequiresRegion,
			expectError:     false,
		},
		{
			name:            "max_children",
			...
			constraintValue: "10",
			expectedType:    RuleTypeMaxChildren,
			expectError:     false,
		},
		{
			name:            "min_children",
			...
			constraintValue: "1",
			expectedType:    RuleTypeMinChildren,
			expectError:     false,
		},
		{
