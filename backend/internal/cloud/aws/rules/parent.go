...
import (
	"context"
	"fmt"
	_ "github.com/mo7amedgom3a/arch-visualizer/backend/internal/resource"
)
...
func (r *RequiresParentRule) GetType() RuleType {
	return RuleTypeRequiresParent
}
...
func (r *RequiresParentRule) Evaluate(ctx context.Context, evalCtx *EvaluationContext) error {
...
func (r *AllowedParentRule) GetType() RuleType {
	return RuleTypeAllowedParent
}
...
func (r *AllowedParentRule) Evaluate(ctx context.Context, evalCtx *EvaluationContext) error {
	if evalCtx.Resource == nil {
		return fmt.Errorf("resource is required for evaluation")
	}
	...
	for _, parent := range evalCtx.Parents {
		...
		if !allowed {
			return &RuleError{
				RuleType:     RuleTypeAllowedParent,
				ResourceID:   evalCtx.Resource.ID,
				ResourceName: evalCtx.Resource.Name,
				ResourceType: r.ResourceType,
